...
def document_node(state: ReportState) -> ReportState:
    ...
    state["stream_updates"].append(
        f"[{timestamp}] Document Agent → Starting document ingestion"
    )
    ...
    try:
        ...
        for pdf_path in uploaded_pdfs:
            state["stream_updates"].append(
                f"[{timestamp}] Document Agent → Processing PDF: {os.path.basename(pdf_path)}"
            )
        ...
        for url in input_urls:
            state["stream_updates"].append(
                f"[{timestamp}] Document Agent → Fetching URL: {url[:50]}..."
            )
        ...
        if all_chunks:
            state["stream_updates"].append(
                f"[{timestamp}] Document Agent → Generating document summary..."
            )
