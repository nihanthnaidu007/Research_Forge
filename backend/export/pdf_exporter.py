...
def on_page(canvas, doc):
    ...
    if page_num > 1:
        ...
        canvas.drawString(
            0.75 * inch, 10.38 * inch, "ResearchForge - Multi-Agent Research Report"
        )
        canvas.drawRightString(
            7.75 * inch, 10.38 * inch, datetime.now().strftime("%B %d, %Y")
        )
...
def build_cover_page(
    ...
    # Subtitle
    depth_label = (
        "Quick Report (3 sections)" if depth == "quick" else "Deep Report (6 sections)"
    )
    story.append(
        Paragraph(
            f"Generated by ResearchForge · {depth_label}", styles["CoverSubtitle"]
        )
    )
    story.append(Spacer(1, 0.1 * inch))
    ...
    # Divider
    story.append(
        HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceAfter=0.4 * inch)
    )
    ...
    stats_data = [
        ...
        [
            Paragraph(
                f'<font color="{_color_hex(conf_col)}">{conf_pct} {conf_lbl}</font>',
                styles["StatValue"],
            ),
...
def build_toc(sections: list[dict], styles) -> list:
    ...
    story.append(
        HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceAfter=0.2 * inch)
    )
...
def build_section(
    ...
    bar_data = [["", ""]]
    bar_table = Table(
        bar_data, colWidths=[bar_filled, 6.0 * inch - bar_filled], rowHeights=[4]
    )
    bar_table.setStyle(
        TableStyle(
            [
    ...
    elements.append(
        HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceAfter=0.3 * inch)
    )
...
def build_citations_page(sources: list[dict], styles) -> list:
    ...
    story.append(Spacer(1, 0.15 * inch))
    story.append(
        HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceAfter=0.25 * inch)
    )
...
def export_report_to_pdf(state: dict, output_path: str) -> str:
    ...
    logger.info(
        f"Generating PDF for topic: {topic} ({len(written_sections)} sections, {len(sources)} sources)"
    )
